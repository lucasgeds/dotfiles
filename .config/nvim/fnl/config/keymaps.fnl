...
(let [opts      {:noremap true :silent true}
  ...
  (nvim.set_keymap :n :<C-l> ":let @+ = expand('%') . '#L' . line('.')<CR>" opts) ; Copy file relative path + line number to + register in Normal Mode
  (nvim.set_keymap :n :<A-c> ":let @+ = expand('%:p')<CR>" opts) ; Copy file absolute path to + register in Normal Mode
